...
void quantize_row_q4_0(const float *restrict x, void *restrict y, int k)
{
    ...
    for (int i = 0; i < nb; i++)
    {
        ...
        for (int l = 0; l < 8; l++)
            srcv[l] = *(vector float *)(x + i * 32 + 4 * l);
        for (int l = 0; l < 8; l++)
            asrcv[l] = vec_abs(srcv[l]);

        for (int l = 0; l < 4; l++)
            amaxv[2 * l] = vec_max(asrcv[2 * l], asrcv[2 * l + 1]);
...
inline static void ggml_vec_dot_q4_0(const int n, float *restrict s, const void *restrict x, const void *restrict y)
{
    ...
    for (int i = 0; i < nb; i += 2)
    {
        ...
        sum0 += d0_0 * d1_0 * (wasm_i16x8_extract_lane(p_0, 0) + wasm_i16x8_extract_lane(p_0, 1) + wasm_i16x8_extract_lane(p_0, 2) + wasm_i16x8_extract_lane(p_0, 3) + wasm_i16x8_extract_lane(p_0, 4) + wasm_i16x8_extract_lane(p_0, 5) + wasm_i16x8_extract_lane(p_0, 6) + wasm_i16x8_extract_lane(p_0, 7));
        sum1 += d0_1 * d1_1 * (wasm_i16x8_extract_lane(p_1, 0) + wasm_i16x8_extract_lane(p_1, 1) + wasm_i16x8_extract_lane(p_1, 2) + wasm_i16x8_extract_lane(p_1, 3) + wasm_i16x8_extract_lane(p_1, 4) + wasm_i16x8_extract_lane(p_1, 5) + wasm_i16x8_extract_lane(p_1, 6) + wasm_i16x8_extract_lane(p_1, 7));
    }

...
BOOL llama_get_model_type(
    ...
    try {
        ...
        if (outError) {
            ...
            *outError = makeLlamaErrorWithUnderlyingError(_LlamaErrorCodeFailedToGetModelType, @"Couldn't determine model type", makeLlamaError(_LlamaErrorCodeGeneralInternalLoadFailure, errorDescription));
        }
        return NO;
    }
}
...
static BOOL llama_model_load(
    ...
    try {
        ...
        if (outError) {
            *outError = makeFailedToLoadModelErrorWithUnderlyingError(makeLlamaError(_LlamaErrorCodeGeneralInternalLoadFailure, [NSString stringWithCString:err.c_str() encoding:NSUTF8StringEncoding]));
        }
        return NO;
    }
}
...
BOOL llama_model_quantize(
  ...
  enum llama_ftype   ftype,
    ...
    try {
        ...
        if (outError) {
            *outError = makeFailedToQuantizeErrorWithUnderlyingError(makeLlamaError(_LlamaErrorCodeGeneralInternalQuantizationFailure, [NSString stringWithCString:err.c_str() encoding:NSUTF8StringEncoding]));
        }
        return NO;
    }
}
